    ...
    <div class="content">
      ...
      <el-row class="form" :gutter="50">
        <el-form style="width: 100%" :inline="true" class="demo-form-inline">
          <el-col :span="5">
            ...
            <el-form-item v-show="dialogTitle == '查看'" label="门店名称">
              <div>张三门店</div>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="5">
            ...
            <el-form-item label="电话咨询" v-show="dialogTitle == '查看'">
              <div>
                {{ telephone }}
              </div>
            </el-form-item>
      ...
      <div class="positionBox">
          ...
          <el-col :span="6">
            ...
            >
              <el-autocomplete
                ...
                placeholder="请输入地理位置"
                @select="wzHandleSelect"
                :trigger-on-focus="false"
                class="w-50 m-2"
              />
              <el-button type="primary" @click="positionBtnClick">
                确定
              </el-button>
            </div>
            <!-- 地图 -->
        ...
        <el-row class="facilitiesBox">
          ...
          <el-col :span="20">
            <div v-show="dialogTitle == '新增人员'" class="positionSeach">
              <el-input
        ...
        <el-row class="facilitiesBox">
          ...
          <el-col :span="20">
            <div class="checkboxContent">
              <el-checkbox-group
                v-model="rentingCheckValue"
                @change="rentingCheckChange"
              >
                <el-checkbox
                  v-show="dialogTitle == '新增人员' || dialogTitle == '编辑'"
                  v-for="item in rentingCheckList"
                  :key="item"
                  :label="item.label"
                  name="type"
                ></el-checkbox>
              ...
              <ul class="ul" v-show="dialogTitle == '查看'">
                <li v-for="item in rentingCheckList" :key="item">
                  {{ item.label }}
                </li>
              </ul>
            </div>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const jsonp = proxy.$jsonp //获取jsonp对象
    ...
    // 位置与周边
    const positionPeriphery = reactive({
      positionValue: '', //位置输入框值
      // 地图搜索框下拉列表
      querySearchAsync: async (queryString, cb) => {
        positionPeriphery.geolocation() //获取当前经纬度
        console.log(queryString, cb)
        try {
          const res = await positionPeriphery.keywordRequest()
          console.log(res, 'keywordRequest')
          if (res.status === 0) {
            if (res.data && res.data.length != 0) {
              ...
              var results = queryString
                ? restaurants.filter(
                    positionPeriphery.createFilter(queryString)
                  )
                : restaurants
      ...
      keywordRequest: () => {
        ...
        const params = {
          output: 'jsonp',
          key: '5N3BZ-CGMCR-NWPWV-WWP25-5ZSH3-DYBDB', //你自己申请的key
          keyword: positionPeriphery.positionValue, //关键字
          region: '常州', //
        }
        let url = `https://apis.map.qq.com/ws/place/v1/suggestion` //关键字搜索
      ...
      // 点击确定按钮
      async positionBtnClick() {
        try {
          const res = await positionPeriphery.keywordRequest()
          console.log(res, 'keywordRequest')
          if (res.status === 0) {
            var restaurants = res.data
            console.log(restaurants[0].title[0])
            var index = restaurants.findIndex(
              item => item.title[0] === positionPeriphery.positionValue[0]
            )
            // console.log(index)
            // console.log(restaurants[index])
            const { lat, lng } = restaurants[index].location
            console.log(lat, lng)
            map.setCenter(new window.TMap.LatLng(lat, lng))
            map.setZoom(17)
          }
      ...
      createFilter(queryString) {
        return restaurant => {
          ...
          return (
            restaurant.title
              .toLowerCase()
              .indexOf(queryString.toLowerCase()) === 0 //以开头匹配关键字
          )
        }
      },
      // 获取当前经纬度
    ...
    // 在租房型
    const rentingCheckBox = reactive({
      rentingCheckValue: [], //多选框选中的值
      rentingCheckList: [
        {
          label: '精选双人房',
          message: 'Please select at least one activity type',
      ...
      rentingCheckChange: e => {
        console.log(e, rentingCheckBox)
      },
    })
    // 地图
    const initMap = () => {
      // console.log(window.TMap)
      // console.log(props.joinTime)
      var center
      var zoom
      //定义地图中心点坐标
      if (props.dialogTitle !== '新增人员') {
        let location = props.newlyAddedState.mapInformation.location
        // console.log(location, 'location')
        center = new window.TMap.LatLng(location.lat, location.lng)
        zoom = 17
      } else {
      ...
      watch(
        ...
        (currenDialogTitle, prevDialogTitle) => {
          ...
          if (currenDialogTitle === '查看') {
            ...
            //更新地图位置
            const { lat, lng } = props.newlyAddedState.mapInformation.location
            console.log(lat, lng)
            map.setCenter(new window.TMap.LatLng(lat, lng))
            map.setZoom(17)
            ...
            //地图默认北京
            map.setCenter(new window.TMap.LatLng(39.98412, 116.307484))
            map.setZoom(7.2)
    ...
    return {
      ...
      ...toRefs(positionPeriphery), //位置与周边
      ...toRefs(facilitiesCheckbox), //配套设施
      ...toRefs(labelCheckbox), //标签介绍
      ...toRefs(rentingCheckBox), //在租房型
      ...toRefs(state),
      ...toRefs(btnState),
    }
  },
